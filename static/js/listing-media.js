(function () {
  ...
  function uploadVideo(root, list, policy, file) {
    ...
    if (file.size > policy.max_video_size_bytes || policy.allowed_extensions.indexOf(ext) === -1 || policy.allowed_mime_types.indexOf(file.type) === -1) {
      state.textContent = 'Not allowed · ' + formatSize(file.size);
    ...
    fetch(root.dataset.uploadUrl, {
      ...
      body: JSON.stringify({listing_id: root.dataset.listingId || null, filename: file.name, content_type: file.type, file_size: file.size})
    }).then(function (r) {
  ...
  function postToR2(upload, file, progress) {
    return new Promise(function (resolve, reject) {
      ...
      xhr.onload = function () { xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error('R2 upload failed.')); };
